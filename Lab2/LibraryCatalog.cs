...
namespace Lab2
{
    public class LibraryCatalog
    {
        ...
        public void ListItems()
        {
            ...
            {
                Console.WriteLine($"{item.GetType().Name} - {item.Title}, Author: {item.Author}, Available: {item.Available}");
            }
        }
        ...
        public void FindItem(string keyword)
        {
            ...
            foreach (var item in found)
            {
               if (item.GetType() == typeof(Book))
                    ...
                    Console.WriteLine($"{item.Title} by {item.Author} - Runtime: {(item as DVD).Runtime} mins - Available: {item.Available}");
               else 
